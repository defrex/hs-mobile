...
class GoogleClosureMinifier(object):
    ...
    def __init__(self, jar_loc, level='SIMPLE_OPTIMIZATIONS', v=False,
                 extra=''):
    ...
    def assign_values(self, js, output_location):
        if js is None or len(js) == 0:
            raise ValueError, 'must provide js'
        self.js = js
    ...
    def minify(self):
        ...
        if type(self.js) == str:    
            ...
            js = ''
            for i, f in enumerate(self.js): 
                js += '--js %s ' % f

        command = ('java -jar %(jar)s '
                    '--compilation_level %(level)s '
                    '%(tmp_loc)s '
                    '--js_output_file %(output_loc)s '
                    '%(extra)s') % {
            'jar': self.jar_loc,
            'level': self.level,
            'tmp_loc': js,
        ...
        if self.v: print 'executing compile command: %s' % command 
        p = Popen(command, shell=True)
        retcode = p.wait()
        if tmp is not None: os.remove(tmp.name)
...
class JavaScriptCompiler(object):
    ...
    def __init__(self, directory, minifier=None, compiled_loc=None, 
                 v=False, recurse=True):
        if not isdir(directory):
            raise AttributeError, ('JavaScriptCompiler object must be '
                                   'passed a valid directory')
    ...
    def make_file_list(self):
        ...
        def search_dir(directory):
            for item in os.listdir(directory):
                ...
                elif item.endswith('.soy'):
                    tmpl_files.append(join(directory, item))
        
        search_dir(self.directory)
        self.tmpl_files = tmpl_files
        ...
        while js_added < len(self.file_list) and cur_depth < search_depth:
            for f in js_files:
                ...
                if deps.startswith('//depends:'):
                    ...
                    if len(dep_files) == 1 and dep_files[0].strip() == 'None':
                        top = True
                    else:
                        found = False 
